...
const RoleModel = ({ show, onClose, role, onUpdate }) => {
  ...
  const handleSave = async (id) => {
    try {
      const response = await axios.patch(`http://localhost:4000/role/update-role/${id}`, {
        rolename: roleName,
      });
      console.log('Role has been updated', response.data);
      onUpdate(response.data); // Call the onUpdate function to update the role list in the parent component
      onClose(); // Close the modal
    } catch (error) {
      console.error('Error in role updating', error);
    }
  };
    ...
    <div className="fixed inset-0 bg-gray-800 bg-opacity-80 flex items-center justify-center">
      <div className="bg-white p-5 rounded-lg shadow-lg w-[30%] h-60">
        <div className='flex justify-between'>
          <h2 className="text-xl font-semibold mb-4 text-center text-blue-500">Edit Role</h2>
        ...
        <form className='flex flex-col gap-4' onSubmit={(e) => { e.preventDefault(); handleSave(role._id); }}>
          <input
            type="text"
            value={roleName}
